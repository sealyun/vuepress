 ...
 */
function multiSide(rootDir, nav, options,pagesLink, currentLevel = 0) {
  const sideBar = {};

  pagesLink.forEach(navItem => {
    if (navItem) {
      sideBar[navItem] = side(join(rootDir, navItem), options);
    }
  });
...
function getConfig(
  ...
  const result = {
    nav: navItems || [],
    sidebar: multipleSideBar && navItems ? multiSide(rootDir, navItems, options,pagesLink) : side(rootDir, options)
  };
